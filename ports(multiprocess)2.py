...
if __name__ == '__main__':
	...
	# 	print(q.get())
	# jobs=[multiprocessing.Process(target=a.worker,args=(q,)) for i in range(30)]
	for i in range(10):
		p=multiprocessing.Process(target=a.worker,args=(q,))
		p.start()
	# list(map(lambda x:x.start(),jobs))
	# p.apply_async(func=a.worker,args=(q,))
	# ri=p.map(a.worker,q)
	q.join()
	print("scan over")
